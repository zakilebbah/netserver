...
class Tiers(db.Model):
    ...
    # already exists ("adresse" avec e, alors que dans user sans e.)
    email = db.Column(db.String(100), unique=True, nullable=True)
    adresse = db.Column(db.String(400), nullable=True) 
    ##
    ...
    def verify_auth_token(token):
        ...
        return  db.session.query(Tiers).filter_by(email=data['email']).first()
...
class Article(db.Model):
    ...
    htmldetail = db.Column(db.String(20000), nullable = True)
    boutiq_visible = db.Column(db.Integer, nullable=True)
...
class Piece(db.Model):
    ...
    chaine17 = db.Column(db.String(1000), nullable=True)
    chaine20 = db.Column(db.String(1000), nullable=True)
